...
function InlineProductCard({ product, side }: { product: ProductLite; side: "left" | "right" }) {
  ...
  return (
    <button
      ...
      className={`${side === "right" ? "sm:float-right sm:ml-4" : "sm:float-left sm:mr-4"} float-none clear-both mx-auto sm:mx-0 mb-3 block w-40 overflow-hidden rounded-xl border border-emerald-400/40 bg-slate-900/70 text-left shadow-lg backdrop-blur-md transition hover:border-emerald-300 sm:w-44`}
        ...
        if (part.type === "image") {
          ...
          return (
            <img
              ...
              src={part.src}
              alt=""
              className={`${side === "right" ? "sm:float-right sm:ml-4" : "sm:float-left sm:mr-4"} float-none clear-both mx-auto sm:mx-0 mb-3 mt-1 block w-48 rounded-xl border border-white/20 shadow-lg sm:w-52`}
            />
          );
        }
