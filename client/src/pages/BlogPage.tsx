...
function InlineProductCard({ product, side }: { product: ProductLite; side: "left" | "right" }) {
  ...
  return (
    <button
      ...
      className={`${side === "right" ? "float-right ml-4" : "float-left mr-4"} clear-both mb-2 block w-36 overflow-hidden rounded-xl border border-emerald-400/40 bg-slate-900/70 text-left shadow-lg backdrop-blur-md transition hover:border-emerald-300 sm:w-44`}
        ...
        if (part.type === "image") {
          ...
          return (
            <img
              ...
              src={part.src}
              alt=""
              className={`${side === "right" ? "float-right ml-4" : "float-left mr-4"} clear-both mb-2 mt-1 block w-40 rounded-xl border border-white/20 shadow-lg sm:w-52`}
            />
          );
        }
